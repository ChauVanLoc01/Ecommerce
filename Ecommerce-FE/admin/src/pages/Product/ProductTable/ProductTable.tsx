...
import { cn, convertCurrentcy } from 'src/utils/utils'

type ProductTableProps = {
  data: ProductListResponse
  refetch: (options?: RefetchOptions) => Promise<QueryObserverResult<AxiosResponse<ProductListResponse, any>, Error>>
  analyticRefetch: (
    options?: RefetchOptions
  ) => Promise<QueryObserverResult<AxiosResponse<ProductAnalyticResponse, any>, Error>>
}

export function ProductTable({ data, refetch, analyticRefetch }: ProductTableProps) {
  ...
  const { control, handleSubmit, register } = useForm<UpdateProductSchema>({
    resolver: yupResolver(update_product_schema)
  })

  const { mutate, isPending } = useMutation({
    mutationFn: (input: { productId: string; body: UpdateProductBody }) => ProductApi.updateProduct(input),
    onSuccess: () => {
      refetch()
      analyticRefetch()
      setOpenDelete(false)
    ...
    onError: () => {
      toast.error('Lỗi xóa sản phẩm không thành công')
    }
  })

  const categoriesResponse = queryClient.getQueryData(['categories']) as AxiosResponse<CategoryResponse>

  const handleUpdateStatusProduct = (productId: string, status: 'ACTIVE' | 'BLOCK') => () =>
    mutate({ productId, body: { status } })
  ...
  const onSubmit: (productId: string) => SubmitHandler<UpdateProductSchema> = (productId: string) => (data) => {
    mutate({
      productId: productId,
      body: data
    })
                      ...
                      <ContextMenu.Content className='rounded-8'>
                        <Dialog.Root open={openDelete} onOpenChange={setOpenDelete}>
                          <Dialog.Trigger>
                            ...
                            >
                              {row.original.status === 'ACTIVE' ? 'Xóa' : 'Khôi phục'}
                            </ContextMenu.Item>
                          </Dialog.Trigger>
                          <Dialog.Content maxWidth='450px' className='rounded-8'>
                            <Dialog.Title>
                              {row.original.status === 'ACTIVE' ? 'Xác nhận xóa sản phẩm' : 'Xác nhận mở khóa sản phẩm'}
                            </Dialog.Title>
                            <Dialog.Description size='2' mb='4'>
                              {row.original.status === 'ACTIVE'
                                ? `Sản phẩm sẽ không thật sự xóa. Bạn vẫn có thể khôi phục lại
                                                            sản phẩm này`
                                : 'Sản phẩm sẽ được mở bán trên sàn'}
                            </Dialog.Description>
                            <Flex gapX='3' justify='end'>
                              ...
                              <Button
                                ...
                                onClick={handleUpdateStatusProduct(
                                  row.original.id,
                                  row.original.status === 'ACTIVE' ? 'BLOCK' : 'ACTIVE'
                                )}
                              >
                                {isPending && <Spinner />}
                                Xác nhận
                        ...
                        <Dialog.Root>
                          ...
                          <Dialog.Content maxWidth='800px' className='rounded-8 space-y-5'>
                            ...
                            <Flex gapX={'5'}>
                              <Avatar src={row.original.image} fallback={'IMG'} size={'9'} radius='small' />
                              <DataList.Root>
                                <DataList.Item align='start'>
                                  <DataList.Label minWidth='95px'>Tên sản phẩm</DataList.Label>
                                  <DataList.Value>{row.original.name}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Mã sản phẩm</DataList.Label>
                                  <DataList.Value>
                                    <Flex align='center' gap='2'>
                                      <Code variant='ghost' className='line-clamp-1'>
                                        {row.original.id}
                                      </Code>
                                ...
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Trạng thái</DataList.Label>
                                  <DataList.Value>
                                ...
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Danh mục</DataList.Label>
                                  <DataList.Value>{categories[row.original.category].name}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Giá hiện tại</DataList.Label>
                                  <DataList.Value>{convertCurrentcy(row.original.priceAfter)}đ</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Giá trước đó</DataList.Label>
                                  <DataList.Value>{convertCurrentcy(row.original.priceBefore)}đ</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Số lượng còn lại</DataList.Label>
                                  <DataList.Value>{convertCurrentcy(row.original.currentQuantity)}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Số lượng đã bán</DataList.Label>
                                  <DataList.Value>{convertCurrentcy(row.original.sold)}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Số lượng ban đầu</DataList.Label>
                                  <DataList.Value>{convertCurrentcy(row.original.initQuantity)}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Số sao đánh giá</DataList.Label>
                                  <DataList.Value>{row.original.rate}</DataList.Value>
                                </DataList.Item>
                                <DataList.Item>
                                  <DataList.Label minWidth='95px'>Thời gian tạo</DataList.Label>
                                  <DataList.Value>{format(row.original.createdAt, 'hh:mm - dd/LL/y')}</DataList.Value>
                                </DataList.Item>
                              </DataList.Root>
                            </Flex>
                          </Dialog.Content>
                        ...
                        <Dialog.Root open={openUpdate} onOpenChange={setOpenUpdate}>
                          ...
                          <Dialog.Content maxWidth='800px' className='rounded-8'>
                            <Dialog.Title className='mb-7'>Cập nhật thông tin sản phẩm</Dialog.Title>
                            <form onSubmit={handleSubmit(onSubmit(row.original.id))}>
                              <Flex gapX={'6'}>
                                <Avatar src={row.original.image} fallback={'IMG'} size={'9'} radius='small' />
                                <DataList.Root>
                                  <DataList.Item align='start'>
                                        ...
                                        render={({ field, fieldState: { error } }) => (
                                          <Flex direction={'column'}>
                                            <Flex direction='column' width='140px'>
                                              ...
                                              >
                                                <Select.Trigger />
                                                <Select.Content position='popper' className='rounded-6'>
                                                  <Select.Group>
                                                    <Select.Label>
                                                      Trạng thái ban đầu: {row.original.status}
                                                    </Select.Label>
                                            ...
                                            <Flex direction={'column'}>
                                              ...
                                              >
                                                ...
                                                <Select.Content>
                                                  <Select.Group>
                                                    <Select.Label>
                                                      Danh mục ban đầu: {categories[row.original.category].name}
                                                    </Select.Label>
                                  ...
                                  <DataList.Item>
                                    ...
                                    <DataList.Value>
                                      <Grid columns={'3'} align={'center'}>
                                        <Text>{convertCurrentcy(row.original.priceAfter)}đ</Text>
                                        <Controller
                                          control={control}
                                          name='priceAfter'
                                          render={({ field, fieldState: { error } }) => (
                                  ...
                                  <DataList.Item>
                                    ...
                                    <DataList.Value>
                                      <Grid columns={'3'} align={'center'}>
                                        <Text>{convertCurrentcy(row.original.priceBefore)}đ</Text>
                                  ...
                                  <DataList.Item>
                                    ...
                                    <DataList.Value>
                                      <Grid columns={'3'} align={'center'}>
                                        <Text>{convertCurrentcy(row.original.initQuantity)}</Text>
                                        <Controller
                                          control={control}
                                          name='initQuantity'
                                          render={({ field, fieldState: { error } }) => (
                                  ...
                                  <DataList.Item>
                                    <DataList.Label minWidth='95px'>Thời gian cập nhật</DataList.Label>
                                    <DataList.Value>
                                      {row.original.updatedAt ? format(row.original.createdAt, 'hh:mm - dd/LL/y') : '_'}
                                    </DataList.Value>
                                  </DataList.Item>
                                </DataList.Root>
                              </Flex>
                              <Flex justify={'end'} gapX={'3'}>
                                ...
                                <Button color='blue' type='submit' className='bg-blue text-white'>
                                  {isPending && <Spinner />}
                                  Thay đổi
                                </Button>
                              </Flex>
